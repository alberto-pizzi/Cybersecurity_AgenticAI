...
- The Snap4City AI model/provider requires network access plus `snap4city_model_credentials.json` or interactive model credentials. This file is unrelated to the account used to log in to the assessed dashboard. The provider is remote and is
...
optional job anonymous; `--auth-only` instead blocks because an authenticated session is required. After initialization, the printed MicroX command
...
in memory for that process; the password is not echoed and `snap4city_model_credentials.json` is not rewritten automatically.
