...
- `reuse_on_authorized_siblings: true` is an explicit opt-in for OIDC/SSO credentials. It does **not** widen authorization and it does **not** copy the primary cookie. It only permits already authorized sibling origins discovered at runtime to try SSO with saved browser state and, when required, the same initially resolved username/password. No second console credential prompt is opened.
...
- `configs/dashboard-test.json`: definition for the authorized `192.168.1.81` / `dashboard-test` test; HTTP/80 with base path `/` is enabled while HTTPS/443 remains disabled until verified. Its authorization block also declares `snap4city.org` as an allowed host suffix, so related Snap4City origins discovered at runtime can enter the assessment without any site-specific crawler rule. Its `dashboard_session` credential can reuse `DASHBOARD_TEST_COOKIE` when an existing primary-origin session is supplied, otherwise it performs the Snap4City/Keycloak browser login with `DASHBOARD_TEST_USERNAME` and `DASHBOARD_TEST_PASSWORD`. The raw primary Cookie header is never copied to a sibling origin. Because this credential explicitly sets `reuse_on_authorized_siblings: true`, authenticated discovery may instead establish an independent sibling session through the real OIDC/SSO flow: it first reuses the saved browser identity-provider state, and if Keycloak asks for credentials again it automatically reuses the same username/password resolved at the beginning of the assessment without prompting again in the orchestrator console.
...
- `configs/platform.example.json`: example of a larger multi-host scope that explicitly demonstrates both supported web-target forms: one service uses an absolute `service.url`, while other services use asset host/IP plus port/protocol/base path. It also demonstrates the credential-reuse contract rather than only naming a shared `credential_ref`: `portal_primary` is a `snap4city_oidc` credential with `reuse_on_authorized_siblings: true` and environment-backed cookie/username/password fields. The two example HTTPS origins reuse the same credential reference but still obtain separate exact-origin application sessions. The unconfirmed example deliberately does not pre-authorize extra runtime sibling origins: for a real assessment, `authorization.confirmed=true` and explicit `allowed_origins`/`allowed_host_suffixes` must be supplied before dynamically discovered siblings can use the same SSO identity. The primary raw Cookie header is never copied cross-origin; saved SSO state and, if required, the username/password resolved once at the beginning of the assessment are reused without a second console prompt. A secondary cookie identity is still shown for authorization comparisons, together with a non-web inventory service and a top-level `reporting` block with `aggregate_report=true` and `keep_job_reports=false`.
...
The compact group catalog shown to the AI is capped at 32/40/48 groups overall and is populated fairly across profiles; with the current registry this is sufficient to expose all profile/tool groups even when anonymous and authenticated are both active. The optional breadth review is also profile-local: when the AI has selected too few **non-baseline** capabilities for one profile, it may add at most 3/6/8 groups in fast/balanced/deep while remaining inside that profile's 12/16/18 total group ceiling. Mandatory baseline groups do not make a profile look artificially complete for this sparse-plan test. The console and planner audit report the concrete pool, group pool, deterministic baseline groups, discretionary AI groups, breadth-review groups, validated concrete actions, base/adaptive concrete capacity and any selected-group actions deferred to a later round. Authorized sibling-origin broad scans use the same shared adaptive origin policy as Deterministic (base 1/3/8 and maximum 2/5/12 in fast/balanced/deep). A sibling without an independently validated session gets one no-cookie broad scan under `anonymous` when that profile exists; a sibling for which runtime OIDC/SSO establishes its own exact-origin session may additionally receive genuine authenticated broad coverage. The primary Cookie header itself is never forwarded cross-origin.

Credential scope is stricter than assessment scope. `authorization.allowed_origins` and `allowed_host_suffixes` decide which origins may be tested, but they do not authorize blindly copying a raw Cookie header. A command-line/config Cookie header does not contain the browser cookie's original `Domain`/`Path`/`Secure` metadata, so that raw value remains confined to the exact primary origin. When OIDC browser state is available, SecOps now preserves and evaluates the real browser cookie metadata for every concrete request URL: host-only cookies remain host-only, legitimate parent-domain or path-scoped cookies are reused only where a browser would send them, and a path-specific application cookie can override a same-name primary cookie for that path. This also handles platforms where several applications share one scheme/host/port but maintain distinct sessions.

For an OIDC credential that explicitly enables `reuse_on_authorized_siblings`, the runner keeps the browser storage state and the username/password resolved once at the beginning of the job. Authorized sibling origins are authenticated independently: failure or credential rejection on one origin no longer suppresses attempts on other authorized origins. The same cached username/password are reused automatically whenever the identity provider asks again, so the child orchestrator never opens another credential prompt. In addition, same-origin application roots that expose their own login/SSO wrapper can receive a separate application session and bounded authenticated re-discovery. If a later scanner precheck still reaches a login page, SecOps performs a just-in-time SSO/reauthentication for that generic application root and rebuilds the scanner arguments with the newly applicable cookie before execution. A failed application login is cached to avoid repeated hammering, but distinct endpoints in the same application can still provide a small bounded retry opportunity. No raw primary `PHPSESSID` is copied to a sibling merely because it is in scope.
...
| Chromium navigations per profile (base / adaptive max) | 36 / 80 | 160 / 400 | 240 / 600 |
| Safe menu/dropdown controls expanded per rendered page | 6 | 18 | 28 |
| JavaScript assets inspected | 32 | 128 | 192 |
...
Traversal uses semantic route identities for routing/file parameters: two requests that share a path and parameter name but route to different internal resources remain distinct. The other request-oriented specialists deliberately use structural route identities, so value-only variants do not multiply SQLMap/Dalfox/Commix/Browser/Workflow work. Traversal additionally has a bounded routing/file reserve of 16/48/96 cases in fast/balanced/deep; these cases are executed before discretionary Agentic round-robin expansion once discovered. The reserve is generic, does not inject target-specific URLs and does not bypass scope or state-change safety checks. Absolute HTTP(S) callback values such as OAuth `redirect_uri=https://...` are not treated as local-file selectors.
...
Snap4City AI provider used by the Agentic planner. Chromium opens the configured target login path; if the anonymous landing page is shown, it activates the visible login control, follows the Keycloak/OIDC flow, submits the supplied account and extracts the target cookies together with the full browser storage state. With `reuse_on_authorized_siblings=true`, authorized sibling origins are attempted independently with that saved SSO state and the same initially resolved username/password when required. The stored Domain/Path/Secure metadata is used to decide which cookies apply to each concrete URL, so same-origin applications can also establish separate path-appropriate sessions. Application roots with an observed login/SSO wrapper receive bounded authenticated re-discovery, and a later authenticated scanner precheck can trigger one just-in-time reauthentication before the scanner arguments are rebuilt. The primary raw `PHPSESSID` is therefore never copied blindly between hosts or assumed to authenticate every application path on one host.
The username/password, browser storage state and cookies are not written to the source configuration or Results Data. To hand the already-resolved authentication state from `assessmentRunner.py` to the child orchestrator, the runner creates a short-lived mode-0600 JSON file, exposes only its path through `SECOPS_RUNTIME_AUTH_STATE`, and deletes the file as soon as that job exits. `DASHBOARD_TEST_COOKIE='PHPSESSID=<SESSION>; ...'` remains a manual primary-origin override: it is never propagated to a different origin. If username/password are also available they can still be reused for sibling OIDC/SSO; if they are unavailable and the saved SSO state cannot authenticate a sibling, that sibling remains anonymous rather than triggering another console prompt. If no account/session is available, leaving the initial console values empty keeps the optional job anonymous; `--auth-only` instead blocks because an authenticated primary session is required. An authenticated profile is eligible for scanner planning only while discovery has not conclusively marked that session ineffective; an invalid supplied cookie therefore cannot be reported as authenticated coverage or suppress the corresponding anonymous coverage. After initialization, `dashboard-test.json` is included automatically in the generic per-config command list: it receives one Deterministic BALANCED command and one Agentic BALANCED command using `--max-rounds 2 --mode balanced --require-ai` and the effective Agentic model (Snap4City or the verified local fallback). The initializer no longer depends on a hardcoded dashboard-test command and no longer prints the isolated ZAP diagnostic command.
