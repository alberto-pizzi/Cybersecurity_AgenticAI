...
Credential scope is stricter than assessment scope. `authorization.allowed_origins` and `allowed_host_suffixes` decide which origins may be tested, but they do not authorize copying a raw Cookie header to those origins. A command-line/config Cookie header no longer contains the browser cookie's original `Domain`/`Path` attributes, so SecOps applies it only to the exact primary origin (scheme + host + port). Session prechecks, runtime preparation, specialist execution and Interactsh injection requests all use the same scoped cookie decision. For an OIDC credential that explicitly enables `reuse_on_authorized_siblings`, the runner also keeps the browser storage state and the username/password that were resolved once at the beginning of the job. When authenticated discovery observes an authorized sibling with application evidence, SecOps opens that sibling in a fresh Chromium context seeded with the saved SSO state. If the identity provider still asks for username/password, the same cached values are filled automatically; the child orchestrator never opens another credential prompt. A successful flow must yield and validate a cookie belonging to that exact sibling origin before any action is treated as authenticated there. The sibling is then re-discovered with its own session so protected menus and request contracts can enter the authenticated surface. If SSO/reauthentication fails or no distinct authenticated state can be demonstrated, the origin stays no-cookie and the equivalent anonymous action is deduplicated instead of being mislabeled authenticated. For separately configured jobs the cache remains keyed by `(credential_ref, exact origin)`, so paths on one origin reuse a session while a different scheme/host/port resolves its own application session.
...
IDOR-Forge is kept in its isolated upstream virtual environment. The initializer now explicitly installs a Python-version-compatible `matplotlib` even when a particular upstream `requirements.txt` revision omits it, and the wrapper performs a lightweight import preflight before a target request. A stale VM therefore reports an actionable dependency diagnostic; rerunning `python initScript.py` without `--skip-scanners` repairs the IDOR-Forge environment.
...
Snap4City AI provider used by the Agentic planner. Chromium opens `/dashboardSmartCity/`; if the anonymous landing page is shown, it activates the visible `login` control, then follows the Keycloak/OIDC flow, submits the supplied account, returns to the dashboard and extracts the target cookies. The browser storage state is retained in memory as well. With `reuse_on_authorized_siblings=true`, an authorized sibling discovered later is opened with that saved SSO state; if Keycloak requires the login form again, the same initially resolved username/password are filled automatically. SecOps accepts the result as authenticated only after obtaining and validating a cookie for that sibling's exact origin, then performs a bounded authenticated re-discovery there. The primary `PHPSESSID` is therefore never copied blindly between hosts.
