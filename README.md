...
| Interactsh actions per round | 1 | 3 | 4 |
...
These two variables are credentials of the **assessed dashboard account** and are unrelated to `snap4city_model_credentials.json`, which authenticates the Snap4City AI provider used by the Agentic planner. The AI provider is separate from target authentication. For target login, Chromium opens the configured application login path and can follow the browser authentication flow. The login flow never widens active-test authorization: a cross-origin identity-provider page may be traversed by the browser, but scanners remain confined to destinations admitted by the configured authorization policy (explicit origins plus the optional same-host/same-scheme multi-port rule). Raw cookies are never copied cross-origin, and the anonymous profile never inherits browser/session state from the authenticated profile.
