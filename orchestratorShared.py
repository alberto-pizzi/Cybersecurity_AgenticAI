...
def _pace_http_request(deadline: float | None = None) -> bool:
    ...
    # without turning per-thread pacers into an accidental multiplier of execution.request_rate.
    return bool(_DISCOVERY_REQUEST_PACER.wait(deadline))
...
MAX_CRAWL_PAGES = max(10, safe_int_value(os.getenv('SECOPS_MAX_CRAWL_PAGES', '10000'), 10000))
MAX_SCRIPT_ASSETS = max(4, safe_int_value(os.getenv('SECOPS_MAX_SCRIPT_ASSETS', '5000'), 5000))
...
ADAPTIVE_SPECIALIST_OVERFLOW = {
    ...
    'balanced': {'arjun': 80, 'sqlmap': 60, 'dalfox': 80, 'commix': 64, 'traversal': 84, 'idor': 48, 'authorization': 90, 'browser': 90, 'workflow': 56},
...
# Per-tool structural variant ceilings. Injection scanners receive the strongest value-only
# consolidation because they mutate the same parameter shape repeatedly; object/authorization and
...
GENERIC_LIVE_INPUT_RESERVE = {
    ...
    'balanced': {'sqlmap': 120, 'dalfox': 160, 'commix': 80},
...
# Very short positive-only cache for just-in-time authenticated prechecks. Consecutive scanner
# actions against the same application family should not immediately repeat target preparation +
# session probing when the exact same scoped cookie was confirmed moments earlier. Failures are
# never cached, and scope/configuration changes clear the cache.
AUTH_SESSION_VALIDATION_CACHE_TTL_SECONDS = min(30.0, max(0.0, safe_float_value(os.getenv('SECOPS_AUTH_SESSION_VALIDATION_CACHE_TTL', '8'), 8.0)))
AUTH_SESSION_VALIDATION_CACHE: dict[tuple[str, str, str, str, bool], tuple[float, dict[str, Any]]] = {}
AUTH_SESSION_VALIDATION_CACHE_LOCK = threading.Lock()
...
BROAD_SCANNER_TIMEOUTS = {name: int(math.ceil(rate_aware_network_budget(value, MAX_REQUEST_RATE))) for name, value in SCAN_MODES[CURRENT_SCAN_MODE]['broad'].items()}
PARAMETER_TOOL_TIMEOUTS = {name: int(math.ceil(rate_aware_network_budget(value, MAX_REQUEST_RATE))) for name, value in SCAN_MODES[CURRENT_SCAN_MODE]['parameter'].items()}
PARAMETER_TOOL_CASE_LIMITS = dict(SCAN_MODES[CURRENT_SCAN_MODE]['limits'])
ARJUN_TIMEOUT = int(math.ceil(rate_aware_network_budget(SCAN_MODES[CURRENT_SCAN_MODE]['arjun'], MAX_REQUEST_RATE)))
...
def _runtime_auth_material_fingerprint(runtime_state: dict[str, Any], cookies: str='') -> str:
    """Hash the identity session material used to validate runtime-auth cache entries.

    Only digests are stored. A changed configured/raw Cookie header or Playwright storage state
    invalidates cached application-auth outcomes while preserving the stable identity reference.
    """
    cookie_fp = _cookie_fingerprint(cookies)
    storage_state = runtime_state.get('storage_state') if isinstance(runtime_state, dict) and isinstance(runtime_state.get('storage_state'), dict) else {}
    if not cookie_fp and not storage_state:
        return ''
    try:
        storage_payload = json.dumps(storage_state, ensure_ascii=False, sort_keys=True, separators=(',', ':'), default=str)
    except (TypeError, ValueError):
        storage_payload = repr(storage_state)
    return hashlib.sha256(f'{cookie_fp}\n{storage_payload}'.encode('utf-8', errors='replace')).hexdigest()
...
def authenticated_application_cookie(candidate: str, identity_cookies: str='') -> str:
    ...
    if not reference:
        return ''
    runtime_state = _runtime_target_auth_for_cookie(identity_cookies)
    session_fingerprint = _runtime_auth_material_fingerprint(runtime_state, identity_cookies)
    ...
    if not isinstance(cached, dict) or cached.get('status') != 'authenticated':
        return ''
    if not session_fingerprint or str(cached.get('session_fingerprint') or '') != session_fingerprint:
        return ''
...
def configure_scan_mode(mode: str) -> None:
    global CURRENT_SCAN_MODE, ARJUN_TIMEOUT, ARJUN_ENDPOINT_LIMIT, SAME_HOST_SERVICE_DISCOVERY_TIME_SPENT_SECONDS, REQUEST_RATE_BUDGET_SCALE
    ...
    BROAD_SCANNER_TIMEOUTS.update({name: int(math.ceil(rate_aware_network_budget(value, MAX_REQUEST_RATE))) for name, value in profile['broad'].items()})
    PARAMETER_TOOL_TIMEOUTS.clear()
    PARAMETER_TOOL_TIMEOUTS.update({name: int(math.ceil(rate_aware_network_budget(value, MAX_REQUEST_RATE))) for name, value in profile['parameter'].items()})
    PARAMETER_TOOL_CASE_LIMITS.clear()
    PARAMETER_TOOL_CASE_LIMITS.update(profile['limits'])
    ARJUN_TIMEOUT = int(math.ceil(rate_aware_network_budget(profile['arjun'], MAX_REQUEST_RATE)))
...
def select_adaptive_final_xss_candidates(ranked: list[tuple[int, dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    if len(ordered) > base and len(selected) >= base and maximum > base:
    ...
    deferred_total = max(0, eligible_total - selected_total)
    for item in selected:
        item['final_xss_eligible_total'] = eligible_total
        item['final_xss_selected_total'] = selected_total
        item['final_xss_deferred_total'] = deferred_total
        item['final_xss_base'] = base
        item['final_xss_max'] = maximum
    return selected
...
async def call_mcp(
    server_file: str, tool_name: str, arguments: dict[str, Any], timeout_seconds: float=MCP_TOOL_TIMEOUT,
    *, hard_timeout_seconds: float | None = None,
    ...
    started = time.monotonic()
    hard_timeout = None if hard_timeout_seconds is None else max(0.0, float(hard_timeout_seconds))
    hard_deadline = (started + hard_timeout) if hard_timeout and hard_timeout > 0 else None

    def _remaining_hard_timeout(default: float) -> float:
        if hard_deadline is None:
            return float(default)
        return max(0.0, min(float(default), hard_deadline - time.monotonic()))
    ...
    try:
        ...
        first_timeout = _remaining_hard_timeout(operation_timeout)
        if first_timeout <= 0:
            raise asyncio.TimeoutError('hard MCP operation deadline reached before invocation')
        try:
            data, is_error, shape = await asyncio.wait_for(invoke(url), timeout=first_timeout)
        except Exception as first_exc:
            if not _http_transport_failure(first_exc):
                raise
            retry_timeout = _remaining_hard_timeout(operation_timeout)
            if retry_timeout <= 0:
                raise asyncio.TimeoutError('hard MCP operation deadline reached before transport retry') from first_exc
            url = await asyncio.to_thread(_ensure_http_server, restart=True)
            retry_timeout = _remaining_hard_timeout(operation_timeout)
            if retry_timeout <= 0:
                raise asyncio.TimeoutError('hard MCP operation deadline reached before restarted transport invocation') from first_exc
            data, is_error, shape = await asyncio.wait_for(invoke(url), timeout=retry_timeout)
...
async def call_mcp_with_progress(spec: ToolSpec, arguments: dict[str, Any], *, timeout_seconds: float=MCP_TOOL_TIMEOUT, hard_timeout_seconds: float | None=None) -> dict[str, Any]:
    ...
    started = time.monotonic()
    task = asyncio.create_task(call_mcp(spec.server, spec.tool, arguments, timeout_seconds=timeout_seconds, hard_timeout_seconds=hard_timeout_seconds))
    while True:
...
# Validates the isolated upstream IDOR-Forge Python environment before a long assessment starts.
# Runtime auto-repair remains disabled: initScript/setupTools owns installation, while preflight
# fails early with an actionable diagnostic if the isolated venv is stale or incomplete.
def _idor_forge_runtime_check() -> tuple[bool, str]:
    runtime = load_runtime_config().get('idor_forge', {})
    configured = runtime if isinstance(runtime, dict) else {}
    root = Path(str(configured.get('directory') or (Path.home() / '.local' / 'opt' / 'idor-forge'))).expanduser()
    python = str(configured.get('python') or '').strip()
    if not python:
        python = str(root / '.venv' / ('Scripts/python.exe' if os.name == 'nt' else 'bin/python'))
    checker = root / 'core' / 'IDORChecker.py'
    if not root.is_dir() or not checker.is_file() or not Path(python).is_file():
        return False, f'IDOR-Forge runtime missing/incomplete: root={root}; python={python}; rerun initScript.py without --skip-scanners.'
    environment = _server_env()
    environment['MPLBACKEND'] = 'Agg'
    contract = (
        "import inspect, matplotlib; from core.IDORChecker import IDORChecker; "
        "init=inspect.signature(IDORChecker.__init__).parameters; "
        "check=inspect.signature(IDORChecker.check_idor).parameters; "
        "need_init={'url','delay','headers','timeout','verbose','max_workers','max_retries','logger'}; "
        "need_check={'test_values','method','max_workers'}; "
        "assert need_init.issubset(init), f'IDORChecker.__init__ missing {sorted(need_init-set(init))}'; "
        "assert need_check.issubset(check), f'IDORChecker.check_idor missing {sorted(need_check-set(check))}'; "
        "assert ({'param','parameter'} & set(check)), 'IDORChecker.check_idor missing object-reference parameter argument'; "
        "assert callable(getattr(IDORChecker,'_generate_payloads',None)), 'IDORChecker._generate_payloads missing'; "
        "print('IDOR-Forge API contract OK')"
    )
    try:
        completed = subprocess.run(
            [python, '-c', contract], cwd=str(root), env=environment,
...
# Validates that the Playwright package can launch its bundled Chromium browser.
# Runtime preflight diagnoses only; initScript/setupTools remains the installer/repair owner.
def _playwright_chromium_runtime_check() -> tuple[bool, str]:
    probe_code = (
    ...
    try:
        completed = subprocess.run(
            [sys.executable, '-c', probe_code], cwd=str(ROOT), env=_server_env(),
            capture_output=True, text=True, encoding='utf-8', errors='replace',
            timeout=45, check=False,
        )
    except Exception as exc:
        return False, f'Playwright Chromium runtime probe failed: {type(exc).__name__}: {exc}'
    detail = '\n'.join(part for part in (completed.stdout or '', completed.stderr or '') if part).strip()
    if completed.returncode != 0:
        return False, (detail[-2500:] or f'Playwright Chromium probe exit={completed.returncode}') + ' | rerun initScript.py to install/repair Chromium.'
    return True, detail[-1200:] or 'Playwright Chromium launch verified.'
...
def run_preflight_checks(*, include_live: bool=True) -> list[dict[str, str]]:
    ...
    for spec in ALL_TOOLS:
        ...
        if spec.name == 'report':
            ...
            checks.append({'level': 'ok' if module_found else 'error' if spec.required else 'warning', 'component': spec.name, 'cause': 'python_dependency_found' if module_found else 'missing_python_dependency', 'detail': spec.module})
            if spec.name == 'browser' and module_found:
                chromium_ok, chromium_detail = _playwright_chromium_runtime_check()
                checks.append({
                    'level': 'ok' if chromium_ok else 'warning',
                    'component': 'browser',
                    'cause': 'playwright_chromium_ready' if chromium_ok else 'missing_playwright_browser',
...
def _request_body_shape(case: dict[str, Any]) -> tuple[Any, ...]:
    ...
    field_names = tuple(sorted({
        str(value).strip().lower() for value in [
            *list(case.get('parameters') or []),
            *list(case.get('file_parameters') or []),
            *list(case.get('token_parameters') or []),
        ] if str(value).strip()
    }))
    if 'json' in content_type and data.strip():
        try:
            ...
            def walk(value: Any, prefix: str='') -> list[str]:
                ...
                if isinstance(value, dict):
                    for key in sorted(value, key=lambda item: str(item)):
                        ...
                        rows.append(child.lower())
                        rows.extend(walk(value[key], child))
                elif isinstance(value, list):
                    rows.append((prefix + '[]').lower())
                    for item in value[:1]:
                        rows.extend(walk(item, prefix + '[]'))
                return rows
...
def _specialist_case_shape(tool: str, case: dict[str, Any]) -> tuple[Any, ...]:
...
def _specialist_variant_cap(tool: str='') -> int:
    name = str(tool or '').lower()
    table = SPECIALIST_TOOL_VARIANT_LIMITS.get(name)
    if table:
        return max(1, int(table.get(CURRENT_SCAN_MODE, SPECIALIST_ROUTE_VARIANT_LIMITS.get(CURRENT_SCAN_MODE, 3))))
...
def _request_with_tls_trust_retry(
    session: requests.Session, method: str, url: str, *, deadline: float | None = None, **kwargs: Any,
) -> tuple[requests.Response, bool, str]:
    """Execute one request and retry the same authorized HTTPS URL only after trust failure.

    When a caller owns a monotonic deadline, both the initial request and trust-only retry are clipped
    to the currently remaining allowance. This prevents one TLS fallback from multiplying a phase
    budget that is supposed to be shared with later discovery/scanner work.
    """
    ...
    try:
        return session.request(method, url, **request_kwargs), False, ''
        ...
        if deadline is not None:
            if not _pace_http_request(float(deadline)):
...
def _safe_crawl_get(
    session: requests.Session, requested: str, target: str, *, timeout: tuple[float, float]=(5, 15),
    max_redirects: int=5, cookies: str='', deadline: float | None = None,
...
class _RequestCaseAccumulator(list[dict[str, Any]]):
    ...
    def append(self, case: dict[str, Any]) -> None:  # type: ignore[override]
        if not isinstance(case, dict):
            return
        key = self._key(case)
        if key is None or key not in self._index:
            if key is not None:
                self._index[key] = len(self)
            super().append(case)
            return
        self.duplicates_consolidated += 1
        existing = self[self._index[key]]
        sources = list(existing.get('discovery_sources') or [])
        for source in (existing.get('discovery_source'), case.get('discovery_source')):
            if source and str(source) not in sources:
                sources.append(str(source))
        if sources:
            existing['discovery_sources'] = sources
        if not existing.get('source_url') and case.get('source_url'):
            existing['source_url'] = case.get('source_url')
        old_response = existing.get('browser_response') if isinstance(existing.get('browser_response'), dict) else {}
        new_response = case.get('browser_response') if isinstance(case.get('browser_response'), dict) else {}
        if new_response.get('observed') and not old_response.get('observed'):
            existing['browser_response'] = dict(new_response)

    def extend(self, values: Any) -> None:  # type: ignore[override]
...
def refresh_authenticated_session_state(
    target: str, cookies: str, probe_url: str='', *, allow_state_changes: bool=False, deadline: float | None=None,
) -> dict[str, Any]:

    selected_probe = probe_url or target
    previously_validated_application = bool(authenticated_application_cookie(target, cookies))
    ...
    if not effective_cookies and runtime_target_auth_available(cookies, target):
        runtime_reauth = ensure_runtime_authenticated_request(target, cookies, selected_probe, deadline=deadline)
    ...
    if not effective_cookies:
        return {
            ...
            'auth_classification': 'auth_unknown',
            'auth_outcome': 'AUTH_NOT_APPLICABLE' if runtime_reauth and runtime_reauth.get('auth_outcome') == 'AUTH_NOT_APPLICABLE' else 'AUTH_FAILED',
    ...
    if AUTH_SESSION_VALIDATION_CACHE_TTL_SECONDS > 0:
        now = time.monotonic()
        with AUTH_SESSION_VALIDATION_CACHE_LOCK:
            cached = AUTH_SESSION_VALIDATION_CACHE.get(cache_key)
            if cached and now - float(cached[0]) <= AUTH_SESSION_VALIDATION_CACHE_TTL_SECONDS:
                result = dict(cached[1])
                result['cache_hit'] = True
                result['cache_age_seconds'] = max(0.0, now - float(cached[0]))
                return result
            # Opportunistic pruning keeps this tiny process-local cache bounded during long scans.
            stale_before = now - max(AUTH_SESSION_VALIDATION_CACHE_TTL_SECONDS * 2.0, 1.0)
            for key, value in list(AUTH_SESSION_VALIDATION_CACHE.items()):
                if float(value[0]) < stale_before:
                    AUTH_SESSION_VALIDATION_CACHE.pop(key, None)
    ...
    if not usable and runtime_target_auth_available(cookies, target) and not (runtime_reauth and runtime_reauth.get('attempted')):
        runtime_reauth = ensure_runtime_authenticated_request(target, effective_cookies, selected_probe, deadline=deadline)
    ...
    auth_outcome = (
        'AUTH_EXPIRED' if not usable and previously_validated_application
        else 'AUTH_NOT_APPLICABLE' if not usable and speculative_raw_cookie
        else 'AUTH_VALID' if usable
        else 'AUTH_FAILED'
    )
    result = {
        'performed': True,
        'auth_outcome': auth_outcome,
        'auth_classification': 'current_identity_valid' if usable else ('different_auth' if auth_outcome == 'AUTH_NOT_APPLICABLE' else 'auth_unknown'),
...
def _browser_network_discovery_impl(target: str, cookies: str, html_urls: list[str], forced_urls: list[str] | None=None, priority_urls: list[str] | None=None, allow_state_changes: bool=False, wall_clock_deadline: float | None=None) -> tuple[list[dict[str, Any]], list[dict[str, Any]], list[str], list[dict[str, Any]], dict[str, Any]]:
    ...
    browser_wall_clock_budget = max(
        ...
        min(
            ...
            rate_aware_network_budget(
                float(navigation_max_budget) * BROWSER_DISCOVERY_SECONDS_PER_BASE_PAGE,
                MAX_REQUEST_RATE, network_fraction=1.0,
            ),
        ),
    )
    configured_browser_deadline = browser_started + browser_wall_clock_budget
    external_deadline = None
    if wall_clock_deadline is not None:
        try:
            external_deadline = float(wall_clock_deadline)
        except (TypeError, ValueError):
            external_deadline = None
    browser_deadline = min(configured_browser_deadline, external_deadline) if external_deadline is not None else configured_browser_deadline
    effective_browser_budget = max(0.0, browser_deadline - browser_started)
    budget_info: dict[str, Any] = {
        'base_budget': navigation_budget, 'max_budget': navigation_max_budget,
        'wall_clock_budget_seconds': round(effective_browser_budget, 3),
        'configured_wall_clock_budget_seconds': round(browser_wall_clock_budget, 3),
    ...
    if time.monotonic() >= browser_deadline:
        ...
        return ([], [], [], [{'url': target, 'type': 'BrowserDiscoveryDeadline', 'message': 'Assessment discovery deadline reached before Chromium discovery could start.'}], budget_info)
    try:
        from playwright.sync_api import sync_playwright
    except Exception as exc:
        ...
        with sync_playwright() as playwright:
            ...
            def route_guard(route: Any) -> None:
                ...
                if route.request.is_navigation_request() and not in_scope:
                    ...
                    elif not _pace_http_request(browser_deadline):
                        budget_info['wall_clock_exhausted'] = True
                        route.abort()
                        return
                    route.continue_()
            ...
            def wait_for_browser_settle(max_ms: int) -> None:
                ...
                stable_hits = 0
                while waited < budget and time.monotonic() < browser_deadline:
                    step = min(quantum, budget - waited)
                    page.wait_for_timeout(max(1, step))
                    waited += step
                    try:
                        raw = page.evaluate("""() => [
                            String(document.readyState || ''),
                            performance.getEntriesByType('resource').length,
                            document.documentElement ? document.documentElement.getElementsByTagName('*').length : 0
                        ]""")
                        signature = (str(raw[0]), int(raw[1]), int(raw[2]))
                    except Exception:
                        previous = None
                        stable_hits = 0
                        continue
                    ready = signature[0] in {'interactive', 'complete'}
                    if ready and previous == signature:
                        stable_hits += 1
                    else:
                        stable_hits = 0
                    previous = signature
...
# Builds a deterministic, target-agnostic port order from runtime evidence.
# No application/benchmark port list is embedded here: configured/observed origins are tried first,
# then the host OS/service database, then a deterministic stratified full-range walk.

def _runtime_service_database_ports() -> list[int]:
    """Rank generic TCP ports from runtime service databases without target-specific hardcoding.

    Nmap's service database includes empirical frequency values. Web-named services receive the
    first tier, then other TCP services are ordered by their runtime frequency when available. This
    improves early breadth when a profile cannot finish the whole candidate range, while the later
    stratified walk still guarantees deterministic full-range coverage for DEEP.
    """
    candidates = [
    ...
    service_line = re.compile(
        r'^\s*([^#\s]+)\s+(\d{1,5})/(tcp|udp)\b(?:\s+([0-9]*\.?[0-9]+))?', re.IGNORECASE
    )
    for path in candidates:
        if not path.is_file():
            continue
        try:
            for line in path.read_text(encoding='utf-8', errors='ignore').splitlines():
                ...
                if not match or match.group(3).lower() != 'tcp':
                    continue
                service_name = str(match.group(1) or '').lower()
                port = int(match.group(2))
                if not 1 <= port <= 65535:
                    continue
                frequency = safe_float_value(match.group(4), 0.0) if match.group(4) else 0.0
                web_named = any(token in service_name for token in ('http', 'https', 'www', 'web'))
                tier = 0 if web_named else 1
                candidate = (tier, -frequency, service_name)
                previous = best.get(port)
    ...
    return [
        port for port, _ in sorted(
            best.items(), key=lambda item: (item[1][0], item[1][1], item[0], item[1][2])
        )
    ]


def _stratified_tcp_port_order(limit: int, preferred_ports: list[int] | None=None) -> list[int]:
    ...
    # Ports already observed/configured for the authorized hostname are the strongest runtime signal.
    ...
    # Runtime service databases provide host-environment knowledge without target-specific ground truth.
...
def _same_host_service_time_limits() -> tuple[float, float]:
    limits = DISCOVERY_LIMITS.get(CURRENT_SCAN_MODE, DISCOVERY_LIMITS['balanced'])
    total_base = max(0.0, float(limits.get('same_host_service_time_budget_seconds', 0) or 0))
    initial_base = max(0.0, float(limits.get('same_host_service_initial_time_budget_seconds', total_base) or total_base))
    # Service discovery is request/connect-count dominated. Scale it fully at low configured rates
    # so candidate caps remain physically achievable instead of becoming aspirational only.
    total = rate_aware_network_budget(total_base, MAX_REQUEST_RATE)
    initial = rate_aware_network_budget(initial_base, MAX_REQUEST_RATE)
...
_SERVICE_DISCOVERY_TLS_CONTEXT = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
_SERVICE_DISCOVERY_TLS_CONTEXT.check_hostname = False
_SERVICE_DISCOVERY_TLS_CONTEXT.verify_mode = ssl.CERT_NONE
...
def _socket_http_probe(
    ...
    def attempt(method: str) -> tuple[bool, str, bool]:
        ...
        try:
            ...
            sock = base
            if use_tls:
                # The classification context is immutable after module initialization and can be
                # reused safely by the bounded service-discovery worker pool. Creating a fresh
                # SSLContext for every candidate added pure CPU overhead on large port sweeps.
                context = _SERVICE_DISCOVERY_TLS_CONTEXT
                try:
...
def _discover_same_host_web_services_locked(
    ...
    empty = {
        ...
        'runtime_observed_port_priority_count': 0,
    }
    ...
    for observed_origin in sorted(AUTHORIZED_SCOPE_ORIGINS):
        try:
            observed = urlparse(str(observed_origin or ''))
            if normalized_hostname(observed.hostname or '') != normalized_hostname(hostname):
                continue
            observed_port = observed.port or (443 if observed.scheme.lower() == 'https' else 80 if observed.scheme.lower() == 'http' else 0)
            if observed_port:
                preferred_ports.append(int(observed_port))
        except (TypeError, ValueError):
            continue
    ...
    # profiles; deeper profiles simply have larger candidate/time budgets.
    def probe_port(index_port: tuple[int, int]) -> dict[str, Any]:
        index, port = index_port
        if time.monotonic() >= deadline:
            return {'index': index, 'port': port, 'deadline': True, 'attempts': 0}
        open_address, attempts, tcp_deferred = _tcp_port_open_any(
            probe_addresses, port, deadline=deadline, start_index=index - 1,
        )
        row: dict[str, Any] = {
            'index': index, 'port': port, 'attempts': attempts, 'tcp_deferred': tcp_deferred,
            'open_address': open_address, 'plain_ok': False, 'tls_ok': False,
            'plain_preview': '', 'tls_preview': '', 'plain_deferred': False, 'tls_deferred': False,
        }
        if tcp_deferred or not open_address:
            return row
        ...
        row.update(
            plain_ok=plain_ok, plain_preview=plain_preview, plain_deferred=plain_deferred,
            tls_ok=tls_ok, tls_preview=tls_preview, tls_deferred=tls_deferred,
        )
        return row
            ...
            for future in done:
                ...
                if plain_ok:
                    ...
                    web_services.append({
                        ...
                        'response_preview': str(row.get('plain_preview') or ''),
                        'http_confirmed': True, 'https_confirmed': bool(tls_ok),
                ...
                if tls_ok:
                    ...
                    web_services.append({
                        ...
                        'response_preview': str(row.get('tls_preview') or ''),
                        'http_confirmed': bool(plain_ok), 'https_confirmed': True,
                        'resolved_address': open_address,
                    })
    ...
    service_inventory = [
        {**row, 'service_class': 'generic_http', 'inventory_only': False}
        for row in unique_services
    ] + [
...
DISCOVERY_METADATA_PATHS = (
    ...
    'v3/api-docs', 'manifest.json', 'site.webmanifest', 'service-worker.js', 'sw.js',
)
...
def _ffuf_application_roots(discovery: dict[str, Any], target_url: str) -> list[str]:
    origin = normalized_origin(target_url)
    if not origin:
        return [target_url]
    root = origin.rstrip('/') + '/'
    limit = max(1, int(FFUF_APPLICATION_ROOT_LIMITS.get(CURRENT_SCAN_MODE, 4)))
    candidates: list[tuple[int, str]] = [(100000, root)]
    seen: set[str] = {root}
    values: list[str] = []
    for key in ('explicit_entry_points', 'html_urls', 'urls', 'browser_navigation_urls'):
    ...
    for case in discovery.get('request_cases', []):
        if isinstance(case, dict):
            values.append(str(case.get('url') or ''))
    for value in values:
...
def _ffuf_runtime_budget_seconds(discovery: dict[str, Any], target_url: str) -> int:
...
def _standard_discovery_metadata_urls(target: str) -> list[str]:
    count = max(0, int(DISCOVERY_METADATA_PATH_COUNTS.get(CURRENT_SCAN_MODE, 6)))
    root = normalized_origin(target).rstrip('/') + '/'
    return [urljoin(root, path) for path in DISCOVERY_METADATA_PATHS[:count]]


def _robots_discovery_urls(text: str, source_url: str) -> list[str]:
    values: list[str] = []
    for raw in str(text or '').splitlines():
        ...
        if not value:
            continue
        if name == 'sitemap':
            values.append(urljoin(source_url, value))
        elif name in {'allow', 'disallow'} and value.startswith('/') and '*' not in value and '$' not in value:
            values.append(urljoin(source_url, value))
    return list(dict.fromkeys(values))


def _sitemap_discovery_urls(text: str, source_url: str) -> list[str]:
    ...
    for node in root.iter():
        if str(node.tag).lower().endswith('loc') and node.text:
...
def _json_discovery_document(text: str) -> Any:
...
def _openapi_request_cases(document: Any, source_url: str, target: str) -> tuple[list[dict[str, Any]], list[str]]:
    if not isinstance(document, dict) or not isinstance(document.get('paths'), dict):
        return [], []
    if not (document.get('openapi') or document.get('swagger')):
        return [], []
    source_origin = normalized_origin(source_url)
    base_url = source_origin.rstrip('/') + '/'
    servers = document.get('servers') if isinstance(document.get('servers'), list) else []
    if servers and isinstance(servers[0], dict):
        candidate = str(servers[0].get('url') or '').strip()
        if candidate:
            resolved = urljoin(source_url, candidate)
            if url_in_authorized_scope(target, resolved):
                base_url = resolved.rstrip('/') + '/'
    elif document.get('basePath'):
        base_url = urljoin(source_origin.rstrip('/') + '/', str(document.get('basePath')).lstrip('/')).rstrip('/') + '/'
    cases: list[dict[str, Any]] = []
    get_urls: list[str] = []
    ...
    for raw_path, path_item in document['paths'].items():
        if not isinstance(path_item, dict):
            continue
        safe_path = re.sub(r'\{[^/{}]+\}', '1', str(raw_path or ''))
        raw_parameters = path_item.get('parameters') if isinstance(path_item.get('parameters'), list) else []
        for method, operation in path_item.items():
            if str(method).lower() not in method_names or not isinstance(operation, dict):
                continue
            parameters = [*raw_parameters, *(operation.get('parameters') if isinstance(operation.get('parameters'), list) else [])]
            ...
            for parameter in parameters:
                ...
                if not name:
                    continue
                if location == 'query' and name not in query_names:
                    query_names.append(name)
                if location in {'query','header','cookie','formdata','body'}:
                    fields.append({'name': name, 'type': str((parameter.get('schema') or {}).get('type') if isinstance(parameter.get('schema'), dict) else parameter.get('type') or 'text'), 'value': ''})
            ...
            for content_type, description in content.items():
                schema = (description or {}).get('schema') if isinstance(description, dict) else {}
                properties = schema.get('properties') if isinstance(schema, dict) and isinstance(schema.get('properties'), dict) else {}
                for name, prop in properties.items():
                    if not any(row.get('name') == str(name) for row in fields):
                        fields.append({'name': str(name), 'type': str(prop.get('type') if isinstance(prop, dict) else 'text'), 'value': ''})
            url = urljoin(base_url, safe_path.lstrip('/'))
            if query_names:
                url = url + ('&' if '?' in url else '?') + urlencode([(name, '') for name in query_names])
            if not url_in_authorized_scope(target, url):
                continue
            upper = str(method).upper()
            case = {'url': url, 'method': upper, 'data': '', 'parameters': query_names + [str(row['name']) for row in fields if str(row.get('name') or '') not in query_names], 'file_parameters': [], 'token_parameters': [], 'fields': fields, 'source_url': source_url, 'discovery_source': 'openapi'}
            cases.append(case)
            if upper == 'GET':
                get_urls.append(url)
    return _dedupe_request_cases(cases), list(dict.fromkeys(get_urls))


def _link_header_urls(value: str, source_url: str) -> list[str]:
    urls: list[str] = []
    for match in re.finditer(r'<([^>]+)>', str(value or '')):
        candidate = urljoin(source_url, match.group(1).strip())
        if candidate not in urls:
            urls.append(candidate)
    return urls


def _source_map_urls(script_text: str, script_url: str) -> list[str]:
    values: list[str] = []
    for match in re.finditer(r'[#@]\s*sourceMappingURL\s*=\s*([^\s*]+)', str(script_text or ''), re.I):
        raw = match.group(1).strip().strip('"\'')
        if raw.startswith('data:'):
            continue
        value = urljoin(script_url, raw)
        if value not in values:
            values.append(value)
    return values
...
def discover_target(
    ...
    external_discovery_deadline = None
    if wall_clock_deadline is not None:
        try:
            external_discovery_deadline = float(wall_clock_deadline)
        except (TypeError, ValueError):
            external_discovery_deadline = None
    test_discovery_deadline = (
        http_discovery_started + float(TEST_DISCOVERY_TIME_BUDGET_SECONDS)
        if CURRENT_SCAN_MODE == 'test' else None
    )
    if external_discovery_deadline is not None and test_discovery_deadline is not None:
        http_discovery_deadline = min(external_discovery_deadline, test_discovery_deadline)
    elif external_discovery_deadline is not None:
        http_discovery_deadline = external_discovery_deadline
    else:
        http_discovery_deadline = test_discovery_deadline

    def discovery_request_timeout(connect_seconds: float, read_seconds: float) -> tuple[float, float]:
        # Preserve configured component caps. `_safe_crawl_get()` clips them against the live
        # monotonic deadline separately for every redirect hop and TLS trust retry.
    ...
    coverage_skipped_keys: set[tuple[str, str, str, str]] = set()
    queue: list[str] = []
    queued: set[str] = set()
    queued_signatures: Counter[tuple[str, str, tuple[str, ...]]] = Counter()
    visited_signatures: Counter[tuple[str, str, tuple[str, ...]]] = Counter()
    ...
    def record_coverage_skip(raw_url: str, reason_code: str, reason: str, *, method: str='GET', source_url: str='') -> None:
        ...
        row_key = (row['method'], row['url'], row['reason_code'], row['source_url'])
        if row_key not in coverage_skipped_keys:
            coverage_skipped_keys.add(row_key)
    ...
    request_cases: list[dict[str, Any]] = _RequestCaseAccumulator()
    client_side_candidates: list[dict[str, Any]] = []
    script_urls: set[str] = set()
    script_endpoint_hints: list[dict[str, str]] = []
    script_endpoint_hint_keys: set[tuple[str, str]] = set()
    ...
    def dequeue_http_candidate(batch_origin_reservations: Counter[str]) -> dict[str, Any] | None:
        ...
        while queue and pages_processed + sum(batch_origin_reservations.values()) < page_max_budget and http_attempts < attempt_budget and http_discovery_time_left():
            ...
            if not forced_requested and visited_signatures[signature] >= variant_limit:
                ...
                continue
            if origin_useful_visits[origin] + batch_origin_reservations[origin] >= per_origin_limit or origin_attempts[origin] >= per_origin_attempt_limit:
                skipped_origin_budget += 1
            ...
            return {
                'requested': requested, 'requested_score': requested_score, 'forced_requested': forced_requested,
                'origin': origin,
            }
        return None

    def fetch_http_candidate(item: dict[str, Any]) -> tuple[dict[str, Any], requests.Response | None, str, str, BaseException | None]:
        worker_session = getattr(http_worker_local, 'session', None)
    ...
    while queue and pages_processed < page_max_budget and http_attempts < attempt_budget and http_discovery_time_left() and not http_adaptive_stopped:
        ...
        for item, response, final, redirect_issue, fetch_error in fetched:
            ...
            if fetch_error is not None:
                errors.append({'url': requested, 'type': type(fetch_error).__name__, 'message': str(fetch_error)})
                continue
            ...
            if final_origin not in origin_fingerprints:
                title_match = re.search(r'<title[^>]*>(.*?)</title>', response.text[:65536], re.I | re.S)
                ...
                origin_fingerprints[final_origin] = {
                    'origin': final_origin, 'status': int(response.status_code),
                    'server': str(response.headers.get('Server') or '')[:160],
                    'content_type': str(response.headers.get('Content-Type') or '')[:160],
                    'title': title, 'service_class': service_class,
                    'redirect_location': str(response.headers.get('Location') or '')[:500],
                }
            path_lower = urlparse(final).path.lower()
            discovery_document = _json_discovery_document(response.text)
            openapi_cases, openapi_get_urls = _openapi_request_cases(discovery_document, final, target)
            if openapi_cases:
                request_cases.extend(openapi_cases)
                for row in openapi_cases:
                    if str(row.get('method') or '').upper() == 'GET' and urlparse(str(row.get('url') or '')).query:
                        parameterized.add(str(row.get('url') or ''))
                for value in openapi_get_urls:
                    if not _destructive_crawl_url(value):
                        enqueue(value, source_url=final)
            metadata_urls: list[str] = []
            if path_lower.endswith('/robots.txt') or path_lower == '/robots.txt':
                metadata_urls.extend(_robots_discovery_urls(response.text, final))
            metadata_urls.extend(_sitemap_discovery_urls(response.text, final))
            metadata_urls.extend(_link_header_urls(str(response.headers.get('Link') or ''), final))
            if discovery_document is not None and not openapi_cases:
                # Manifests/config JSON often expose same-origin API roots and routes as ordinary strings.
                metadata_urls.extend(str(hint.get('url') or '') for hint in _literal_navigation_hints(response.text, final, target))
            for value in metadata_urls:
                if value and url_in_authorized_scope(target, value) and not _destructive_crawl_url(value):
                    enqueue(value, source_url=final)
            ...
            for hint in _literal_navigation_hints(response.text, final, target):
                ...
                literal_key = (str(literal_hint.get('method') or 'GET'), str(literal_hint.get('url') or ''))
                if literal_key not in script_endpoint_hint_keys:
                    script_endpoint_hint_keys.add(literal_key)
            ...
            batch_limit = min(len(ranked_scripts), remaining_attempt_slots, remaining_useful_slots)
            if not http_discovery_time_left():
                batch_limit = 0
            script_batch = ranked_scripts[:batch_limit]
            deferred_script_urls.update(ranked_scripts[batch_limit:])
            attempted_script_urls.update(script_batch)
            asset_worker_local = threading.local()

            def fetch_script_asset(script_url: str) -> tuple[requests.Response | None, str, str, BaseException | None]:
            ...
            for script_url, (script_response, script_final, script_issue, script_error) in zip(script_batch, script_results):
                ...
                for hint in [*script_hints, *generic_hints]:
                    ...
                    recorded_key = (str(recorded_hint.get('method') or 'GET'), hinted_url)
                    if hinted_url and recorded_key not in script_endpoint_hint_keys:
                        script_endpoint_hint_keys.add(recorded_key)
            ...
            while source_map_cursor < len(source_map_candidates) and http_discovery_time_left():
                ...
                for (map_url, source_script), map_response, map_final, map_issue, map_error in map_results:
                    ...
                    if map_response is None or map_issue or map_response.status_code >= 400:
                        continue
                    map_final = _clean_url(map_final)
                    if not url_in_authorized_scope(target, map_final):
                        continue
                    document = _json_discovery_document(map_response.text)
                    if not isinstance(document, dict):
                        continue
                    scanned_script_urls.add(map_final)
                    script_urls.add(map_final)
                    embedded = document.get('sourcesContent') if isinstance(document.get('sourcesContent'), list) else []
                    for source_index, source_text in enumerate(embedded[:256]):
                        ...
                        for hint in [*_javascript_endpoint_hints(source_text, source_script, target), *_literal_navigation_hints(source_text, source_script, target)]:
                            ...
                            if hinted_url and key not in script_endpoint_hint_keys:
                                script_endpoint_hint_keys.add(key)
                                script_endpoint_hints.append({'method': hint_method, 'url': hinted_url, 'source': 'javascript_source_map'})
                            params = _query_parameter_names(hinted_url)
                            if params and hint_method in {'GET', 'POST'}:
                                request_cases.append({'url': hinted_url, 'method': hint_method, 'data': '', 'parameters': params, 'file_parameters': [], 'token_parameters': [], 'fields': [], 'source_url': virtual_source, 'discovery_source': 'javascript_source_map'})
                                if hint_method == 'GET':
                                    parameterized.add(hinted_url)
                            if hint_method == 'GET' and hinted_url and not _destructive_crawl_url(hinted_url):
                                enqueue(hinted_url, source_url=virtual_source)
    ...
    browser_cases, browser_network_requests, browser_navigation_urls, browser_errors, browser_budget_info = _browser_network_discovery(
        ...
        allow_state_changes=allow_state_changes, wall_clock_deadline=external_discovery_deadline,
    ) if html_urls or priority_seed_urls else ([], [], [], [], {'base_budget': int(limits['browser_pages']), 'max_budget': int(limits.get('browser_pages_max', limits['browser_pages'])), 'attempted': 0, 'adaptive_overflow_used': 0, 'remaining_candidates': 0, 'adaptive_threshold': None, 'max_saturated': False, 'wall_clock_budget_seconds': 0.0, 'wall_clock_elapsed_seconds': 0.0, 'wall_clock_exhausted': bool(external_discovery_deadline is not None and time.monotonic() >= external_discovery_deadline)})
    ...
    if cookies and (external_discovery_deadline is None or time.monotonic() < external_discovery_deadline):
        probe_url = _stable_auth_probe_url(target, sorted(html_urls))
        try:
            original_headers = dict(session.headers)
            session.headers['Cache-Control'] = 'no-cache'
            probe_response, probe_final, probe_redirect_issue = _safe_crawl_get(session, probe_url, target, timeout=discovery_request_timeout(5, 15), max_redirects=5, cookies=cookies, deadline=http_discovery_deadline)
            ...
            anonymous_response, anonymous_final, anonymous_redirect_issue = _safe_crawl_get(anonymous_session, probe_url, target, timeout=discovery_request_timeout(5, 15), max_redirects=5, cookies='', deadline=http_discovery_deadline)
        ...
        auth_effective = None
        auth_note = 'Authentication comparison was deferred because the assessment discovery deadline was reached.'
        auth_probe = {'conclusive': False, 'diagnosis': 'assessment_discovery_deadline_exhausted'}

    case_source_counts: Counter[str] = Counter()
    for case in request_cases:
        if not isinstance(case, dict):
            continue
        sources = list(case.get('discovery_sources') or [])
        if case.get('discovery_source'):
            sources.append(str(case.get('discovery_source')))
        for source in set(str(value) for value in sources if str(value)):
            case_source_counts[source] += 1
    rate_stats = _DISCOVERY_REQUEST_PACER.stats()
    budget_diagnostics = {
        'mode': CURRENT_SCAN_MODE,
        'request_rate_configured': float(rate_stats.get('configured_rate', MAX_REQUEST_RATE)),
        'request_rate_effective_shared': float(rate_stats.get('last_effective_shared_rate', MAX_REQUEST_RATE)),
        'request_rate_paced_starts': int(rate_stats.get('request_starts', 0) or 0),
        ...
        'http_wall_clock_budget_seconds': max(0.0, http_discovery_deadline - http_discovery_started) if http_discovery_deadline is not None else 0.0,
        'assessment_deadline_applied': bool(external_discovery_deadline is not None),
        ...
        'origin_fingerprints': [origin_fingerprints[key] for key in sorted(origin_fingerprints)],
...
def _runtime_auth_probe(origin: str, cookie: str, probe_url: str, *, deadline: float | None=None) -> dict[str, Any]:
    ...
    try:
        ...
        response = request_same_origin_redirects(
            ...
            timeout=(4, 12), deadline=deadline, allow_state_changes=False,
        )
        anonymous = {
...
def ensure_runtime_authenticated_request(request_url: str, current_cookie: str='', probe_url: str='', timeout_seconds: int | None=None, *, deadline: float | None=None) -> dict[str, Any]:
    ...
    cached_session_fingerprint = str(cached.get('session_fingerprint') or '') if isinstance(cached, dict) else ''
    cache_matches_session = bool(session_fingerprint and cached_session_fingerprint == session_fingerprint)
    cached_cookie = scope_cookie_header(url, current_cookie)
    if cache_matches_session and isinstance(cached, dict) and cached.get('status') == 'authenticated' and cached_cookie:
    ...
    if deadline is not None:
        ...
        if remaining < float(login_floor):
            return {
                'attempted': False, 'usable': False, 'status': 'time_budget',
                'auth_classification': 'auth_unknown', 'auth_outcome': 'AUTH_FAILED',
                'identity_ref': identity_ref, 'scope_key': scope_key, 'request_url': url,
                'reason': 'assessment execution deadline is too near for bounded runtime authentication',
                'diagnosis': 'assessment_time_budget_exhausted', 'cookie_header': '',
                'session_fingerprint': session_fingerprint, 'attempt_count': previous_attempts,
                'attempted_urls': sorted(previous_urls),
            }
        configured_timeout = min(configured_timeout, max(login_floor, int(remaining)))
    credential['timeout_seconds'] = max(login_floor, configured_timeout)
...
def authenticate_discovered_sibling_origins(discovery: dict[str, Any], target: str, primary_cookies: str, *, allow_state_changes: bool=False, wall_clock_deadline: float | None=None) -> dict[str, Any]:
    ...
    runtime_rows: list[dict[str, Any]] = [
        row for row in list(discovery.get('runtime_sibling_authentication') or [])
        if not (
            isinstance(row, dict)
            and str(row.get('status') or '') in {'limit', 'time_budget'}
            and not str(row.get('origin') or '')
        )
    ]
    known_authenticated = {
    ...
    for row in runtime_rows:
        if not isinstance(row, dict) or str(row.get('status') or '') not in {'failed', 'failed_validation', 'no_auth_entry_observed', 'session_only_credentials_required'}:
            continue
    ...
    if wall_clock_deadline is not None:
        try:
            auth_pass_deadline = min(auth_pass_deadline, float(wall_clock_deadline))
        except (TypeError, ValueError):
            pass
    ...
    for origin, score, candidates in selected:
        ...
        try:
            ...
            runtime_rows.append({'origin': origin, 'status': status, 'reason_code': reason_code, 'reason': diagnostic, 'detail': message[:1000], 'score': score, 'candidate_count': len(candidates), 'attempted_candidates': attempted_candidates})
        ...
        probe = _runtime_auth_probe(origin, sibling_cookie, probe_url, deadline=auth_pass_deadline)
        flow_observed = safe_bool_metadata(login.get('authentication_flow_observed'), False)
        ...
        try:
            recrawl = discover_target(
                ...
                allow_state_changes=allow_state_changes, wall_clock_deadline=auth_pass_deadline,
            )
    ...
    application_rows: list[dict[str, Any]] = [
        row for row in list(discovery.get('runtime_application_authentication') or [])
        if not (
            isinstance(row, dict)
            and str(row.get('status') or '') in {'limit', 'time_budget'}
            and not str(row.get('application_scope') or '')
        )
    ]
    ...
    for scope_key, score, candidates in app_ranked[:app_limit]:
        ...
        result = ensure_runtime_authenticated_request(
            entry_url, primary_cookies, entry_url, timeout_seconds=max(2 if CURRENT_SCAN_MODE == 'test' else 15, min(60, int(remaining_auth))), deadline=auth_pass_deadline
        )
    ...
    app_deferred_by_limit = max(0, len(app_ranked) - app_limit)
    if app_deferred_by_limit:
        application_rows.append({
            'status': 'limit',
            'eligible_applications': len(app_ranked),
            'proactive_limit': app_limit,
            'deferred_applications': app_deferred_by_limit,
            'message': 'Additional application scopes remain eligible for just-in-time authentication fallback beyond the proactive runtime-auth application limit.',
        })
    ...
    aggregate_recrawl_budget = dict(merged.get('budget_diagnostics') or {})
    merged['budget_diagnostics'] = {
        **primary_budget,
        'runtime_auth_aggregate_budget_diagnostics': aggregate_recrawl_budget,
        ...
        'runtime_auth_applications_deferred_by_limit': app_deferred_by_limit,
    }
    return merged
...
def expand_discovered_authorized_host_services(
    ...
    if wall_clock_deadline is not None:
        try:
            external_expansion_deadline = float(wall_clock_deadline)
        except (TypeError, ValueError):
            external_expansion_deadline = None
    ...
    for index, (host, origin, score) in enumerate(selected):
        ...
        try:
            ...
            recrawl = discover_target(
                ...
                expand_authorized_service_hosts=False, same_host_service_candidate_cap=0,
                allow_state_changes=allow_state_changes, wall_clock_deadline=external_expansion_deadline,
            )
...
def _object_reference_locations(case: dict[str, Any]) -> list[tuple[str, str, str, str]]:
    """Return runtime-derived object references from query, path segments and form/JSON bodies."""
    rows: list[tuple[str, str, str, str]] = [
        ('query', name, value, mutated) for name, value, mutated in _object_reference_pairs(str(case.get('url') or ''))
    ]
    url = str(case.get('url') or '')
    try:
        segments = [value for value in urlparse(url).path.split('/') if value]
    except ValueError:
        segments = []
    for index, value in enumerate(segments):
        mutated = _mutated_object_reference('id', value)
        if mutated is not None and mutated != value:
            rows.append(('path', f'path[{index}]', value, mutated))
    raw = str(case.get('data') or '')
                ...
                if isinstance(value, dict):
                    for key, nested in value.items():
                        ...
                        if isinstance(nested, (str, int)):
                            original = str(nested)
                            mutated = _mutated_object_reference(str(key), original)
                            if mutated is not None and mutated != original:
                                rows.append(('json', name, original, mutated))
                        else:
                            walk(nested, name, depth + 1)
                elif isinstance(value, list):
                    for index, nested in enumerate(value[:40]):
                        walk(nested, f'{prefix}[{index}]', depth + 1)
            walk(payload)
        else:
            for name, value in parse_qsl(raw, keep_blank_values=True):
                mutated = _mutated_object_reference(name, value)
                if mutated is not None and mutated != value:
                    rows.append(('form', str(name), str(value), mutated))
    return rows
...
def _tool_case_priority(tool: str, case: dict[str, Any], authenticated_profile: bool=False) -> int:
    ...
    if tool == 'traversal':
        ...
        return max(score, 12) if anonymous_login_flow else score
    if tool == 'idor':
        object_pairs = _object_reference_locations(case)
...
def tool_case_hard_skip_reason(tool: str, case: dict[str, Any]) -> str:
    ...
    if name == 'idor':
        ...
        url = str(case.get('url') or '')
        path = urlparse(url).path.lower()
        if method not in {'GET', 'POST'} or not _object_reference_locations(case):
            return 'IDOR requires a safe GET/POST request containing a concrete query, path, form or JSON object-reference value.'
...
def _append_generic_live_input_reserve(
    ...
    for case in output:
        ...
        known.add(key)
        shape_counts[_specialist_case_shape(tool, case)] += 1
    ...
    for index, raw in enumerate(candidates):
        ...
        shape = _specialist_case_shape(tool, case)
...
def select_tool_request_cases(discovery: dict[str, Any], tool: str, limit: int | None=None, authenticated_profile: bool=False, allow_state_changes: bool=False, credential_cookies: str='', *, agentic_catalog: bool=False) -> list[dict[str, Any]]:
    ...
    if agentic_catalog:
        # Agentic planning may choose among more contracts than the deterministic fixed selector, but
        # it must not receive an unbounded number of value-only variants of the same request shape.
        # The exact request is still preserved locally; only structurally redundant variants are
        # bounded here. Traversal keeps semantic routing values distinct through its route signature.
        ...
        for heuristic, case in fair_rows:
            ...
            clean = {k: v for k, v in case.items() if k != '_agentic_source_index'}
            catalog.append({**clean, 'priority_score': int(heuristic), 'selection_reason': 'agentic-structurally-compatible-candidate'})
        return _append_generic_live_input_reserve(
            tool, catalog, cases,
            authenticated_profile=authenticated_profile, credential_cookies=credential_cookies,
        )
...
# Structural context for completion accounting. The completion sweep is intentionally
# value-insensitive so ordinary value-only variants collapse to one tested request shape;
# specialist selectors separately preserve wider semantic-routing/object-reference variants where needed.
def _safe_surface_context_key(case: dict[str, Any]) -> tuple[Any, ...]:
    method = str(case.get('method') or 'GET').upper()
    ...
    # Completion accounting is intentionally value-insensitive. It answers whether this request
    # *shape* received a safe replay; it must not spend thousands of slots on ID/filter values.
    return method, _structural_route_signature(url), tuple(sorted(str(value).lower() for value in params if str(value))), _request_body_shape(case)


def _tested_surface_contexts(profile_results: dict[str, Any]) -> set[tuple[Any, ...]]:
...
def run_safe_surface_sweep(
    target: str, discovery: dict[str, Any], profile_results: dict[str, Any], cookies: str='', *,
    ...
    started = time.monotonic()
    local_deadline = started + total_timeout
    deadline = min(local_deadline, float(deadline)) if deadline is not None else local_deadline
    total_timeout = max(0.0, deadline - started)
...
def select_browser_request_cases(discovery: dict[str, Any], limit: int | None=None, *, agentic_catalog: bool=False) -> list[dict[str, Any]]:
    ...
    if agentic_catalog:
        ...
        for score, case in fair_ranked:
            ...
            params = tuple(sorted(str(value).lower() for value in case.get('parameters', []) if str(value)))
            key = (method, _semantic_request_url_key(url), str(case.get('data') or ''), params)
            shape = (method, _structural_route_signature(url), params)
            if key in seen_catalog or shape_counts[shape] >= variant_cap:
                continue
            seen_catalog.add(key)
            shape_counts[shape] += 1
...
def select_arjun_request_cases(discovery: dict[str, Any], target: str, limit: int=MAX_ARJUN_ENDPOINTS, *, allow_state_changes: bool=False, agentic_catalog: bool=False) -> list[dict[str, Any]]:
    ...
    if agentic_catalog:
        ...
        for raw_case in discovery.get('request_cases', []):
            ...
            params = tuple(sorted(str(value).lower() for value in raw_case.get('parameters', []) if str(value)))
            shape = (method, _structural_route_signature(url), params)
            if key in seen_catalog or shape_counts[shape] >= variant_cap:
                continue
            seen_catalog.add(key)
...
def select_authenticated_precheck_probe_url(
    discovery: dict[str, Any], request_url: str, source_url: str='', method: str='GET',
) -> str:
    """Choose the concrete request only when it is a proven-stable HTML auth probe.

    Parameterized/API GETs can legitimately return 4xx despite a valid session. They therefore must
    not be used as the sole authentication-health check. Exact GET/HEAD probing is reserved for a
    discovery-observed, non-parameterized, non-mutating HTML page; all other request contracts fall
    back to a safe page from the same generic application scope.
    """
    request = str(request_url or '').strip()
    source = str(source_url or '').strip()
    method_upper = str(method or 'GET').upper()
    try:
        normalized_request = normalize_url(request)
    except Exception:
    ...
    for value in discovery.get('html_urls', []):
        if not isinstance(value, str):
            continue
        try:
            discovered_html.add(normalize_url(value))
        except Exception:
            discovered_html.add(value)
    try:
        has_query = bool(urlparse(request).query)
    except ValueError:
        has_query = True
    exact_is_stable = (
        ...
        and bool(request)
        and not has_query
        and normalized_request in discovered_html
        and not request_contract_state_change_reason({
            'url': request, 'method': 'GET', 'data': '', 'parameters': [],
        })
    )
    return request if exact_is_stable else select_application_session_probe_url(discovery, request, source)
