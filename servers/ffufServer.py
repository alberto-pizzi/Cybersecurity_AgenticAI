...
from scannerCommon import looks_like_login, read_json, service

mcp, _serve = service("FFUF Scanner", "ffuf")
    ...
    for attempt in range(max(1, attempts)):
        try:
            ...
            if redirect_guard.startswith(("destructive_", "cross_origin_")):
                return {
                    "performed": True, "authenticated": None, "conclusive": False, "transient_error": False,
                    "status": response.status_code, "final_url": final_url,
                    "login_detected": False, "redirect_guard": redirect_guard, "attempt_errors": errors,
                }
            login = looks_like_login(response, text_limit=50_000, paths=("/login", "/login.php"), words=("login",), strip_trailing_slash=False)
            return {
                "performed": True, "authenticated": response.status_code < 400 and not login,
                "conclusive": True, "transient_error": False, "status": response.status_code, "final_url": str(response.url),
                "login_detected": login, "redirect_guard": redirect_guard, "attempt_errors": errors,
            }
            ...
            break
    return {
        "performed": True, "authenticated": None, "conclusive": False, "transient_error": True,
...
def _safe_verification_get(
    ...
    for _ in range(max(0, int(max_redirects)) + 1):
        ...
        response = requests.get(
            current, headers=headers, timeout=(3, 7), allow_redirects=False,
        )
        ...
        candidate_key = (
            parsed.scheme.lower(), parsed.hostname or "", parsed.port or (443 if parsed.scheme.lower() == "https" else 80),
        )
...
def _verify(url: str, cookies: str) -> dict[str, Any]:
    ...
    try:
        ...
        return _verification_result(
            'observation', 'info', 'Sensitive-looking path could not be verified', 'verification-request-failed',
            'FFUF discovered a sensitive-looking path, but the follow-up verification request failed.',
            confidence='low', evidence=f'Verification request failed: {type(exc).__name__}: {exc}',
        )
    ...
    if redirect_guard.startswith(("destructive_", "cross_origin_")):
        return _verification_result(
            'observation', 'info', 'FFUF redirect blocked by session-safety guard', 'unsafe-redirect-not-followed',
            'The discovered path redirected to a destructive or cross-origin destination. The redirect was not followed with the authenticated cookie.',
    ...
    if login_page and final_url.rstrip("/") != url.rstrip("/"):
        return _verification_result(
            'observation', 'info', 'Sensitive-looking path redirected to authentication', 'access-controlled-or-session-redirect',
            'The discovered path redirected to a login page during verification; exposure was not confirmed.',
            final_url=final_url, final_status=response.status_code, evidence=evidence,
        )

    if response.status_code in {401, 403}:
        return _verification_result(
            'observation', 'info', 'Sensitive-looking path is access controlled', 'access-control-observed',
            f'The verification request returned HTTP {response.status_code}; public exposure was not demonstrated.',
            final_url=final_url, final_status=response.status_code, evidence=evidence,
        )

    if response.status_code >= 400:
        return _verification_result(
            'discovery', 'info', 'FFUF path not confirmed by verification', 'verification-negative',
            f'The follow-up request returned HTTP {response.status_code}; the initial FFUF match was not treated as an exposure.',
            final_url=final_url, final_status=response.status_code, evidence=evidence,
        )

    if original_path.endswith("/.git/head") and sample.lstrip().startswith("ref:"):
        return _verification_result(
            'vulnerability', 'high', 'Exposed Git repository metadata', 'content-verified',
            'The .git/HEAD reference was retrieved from the web server.',
    ...
    if original_path.endswith("/.env") and any(
        ...
        return _verification_result(
            'vulnerability', 'high', 'Environment secrets file exposed', 'content-verified',
            'The response resembles an environment file and contains secret-bearing configuration keys.',
            ...
            final_url=final_url, final_status=response.status_code,
    ...
    if "phpinfo" in original_path and "php version" in lower:
        return _verification_result(
            'vulnerability', 'medium', 'PHP diagnostic page exposed', 'content-verified',
            'A PHP information page was retrieved and disclosed runtime and server configuration details.',
    ...
    if original_path.endswith("server-status") and "apache server status" in lower:
        return _verification_result(
            'vulnerability', 'medium', 'Apache server-status exposed', 'content-verified',
            'The Apache server-status page was retrieved.',
    ...
    if "<title>index of" in lower or "<h1>index of" in lower:
        return _verification_result(
            'candidate', 'low', 'Directory listing reachable', 'content-verified-directory-index',
            'The response contains a directory index for a sensitive-looking path.',
    ...
    if any(token in original_path for token in ("setup", "install")) and (
        ...
        return _verification_result(
            'candidate', 'medium', 'Setup or installation interface reachable', 'content-verified-requires-manual-validation',
            'A setup or installation interface was reachable and contained an actionable form.',
            ...
            confidence='medium', final_url=final_url,
            final_status=response.status_code, evidence=evidence + '\nSetup/installation form markers were detected.',
        )

    return _verification_result(
        'discovery', 'info',
        'Sensitive-looking path reachable without verified sensitive content', 'reachable-content-not-sensitive',
        'The path was reachable, but the verification response did not match a supported sensitive-content signature.',
...
def _finding(item: dict[str, Any], cookies: str, verify: bool) -> dict[str, Any]:
    ...
    if sensitive and verify:
        ...
        return {
            "alert": verified["alert"], "risk": verified["risk"],
            "category": verified["category"], "verification_status": verified["verification_status"],
            "description": verified["description"], "impact": verified["impact"], "solution": verified["solution"], "url": url,
            "final_url": verified.get("final_url", ""), "status_code": status,
            "verification_status_code": verified.get("final_status", 0), "content_length": item.get("length"),
            "words": item.get("words"), "lines": item.get("lines"),
            "redirect_location": item.get("redirectlocation", ""), "confidence": verified["confidence"],
            "evidence": base_evidence + "\n" + verified["evidence"],
        }

    if sensitive:
        return {
            "alert": "Sensitive-looking resource discovered", "risk": "info",
            "category": "discovery", "verification_status": "not-content-verified",
            "description": "FFUF discovered a sensitive-looking path, but it was outside the bounded content-verification subset.",
            "impact": "", "solution": "", "url": url, "status_code": status,
            "content_length": item.get("length"), "words": item.get("words"),
            "lines": item.get("lines"), "redirect_location": item.get("redirectlocation", ""),
            "confidence": "low", "evidence": base_evidence,
        }

    return {
        "alert": "Discovered web resource", "risk": "info", "category": "discovery", "verification_status": "discovery-only",
        "description": f"FFUF received HTTP {status} for this resource.", "impact": "", "solution": "", "url": url,
        "status_code": status, "content_length": item.get("length"), "words": item.get("words"), "lines": item.get("lines"),
...
def run_ffuf_fuzz(target_url: str, cookies: str="", wordlist: str="", timeout: int=120, session_probe_url: str="") -> dict:
    ...
    with tempfile.TemporaryDirectory(prefix="ffuf-priority-") as td:
        ...
        if authenticated and session_before.get("performed") and session_before.get("conclusive") and session_before.get("authenticated") is False:
            return partial(
                "FFUF", target_url, "Authenticated FFUF was not started because the supplied session was already invalid.",
        ...
        common={
            ...
            "authenticated":bool(cookies), "credentialed_fuzz_requests_sent":False,
            "cookie_isolated_discovery":bool(cookies), "authenticated_verification_used":bool(cookies),
            "blocked_destructive_rows":sorted(set(blocked_rows)), "hard_failure":bool(hard),
            "phases":[
                {
                    "name":p.get("phase"), "status":p.get("status"),
                    "findings":p.get("phase_findings",0), "timeout_seconds":p.get("phase_timeout_seconds"),
                }
                for p in phases
            ],
        ...
        if authenticated and session_after.get("performed") and session_after.get("conclusive") and session_after.get("authenticated") is False:
            return partial(
                ...
                diagnosis="authentication_lost_during_ffuf", timed_out=False, **common,
            )
