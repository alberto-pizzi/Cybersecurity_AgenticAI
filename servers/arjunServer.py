...
from scannerCommon import read_json, service, unique_strings

mcp, _serve = service("Arjun Scanner", "arjun")

PRIORITY_PARAMETERS = (
    "id", "uid", "user_id", "account", "role", "admin", "action", "cmd", "command", "exec", "shell", "ip", "host", "file", "path",
    "page", "include", "template", "url", "redirect", "callback", "webhook", "xml", "debug", "download", "upload", "message",
)
...
def _run(target_url: str, cookies: str, output: Path, wordlist: str, budget: int, name: str, method: str = "GET", data: str = "") -> tuple[dict[str,Any],list[str]]:
    method = method.upper()
    command=[
        "arjun", "-u", target_url, "-m", method, "-oJ", str(output), "-q", "-t", "2", "-T", "4",
...
def _finding(target_url: str, parameter: str, method: str = "GET") -> dict[str,Any]:
    ...
    return {
        ...
        "risk": "info", "category": "discovery", "verification_status": "parameter-name-discovery-only",
        ...
        "url": target_url, "method": method, "parameter": parameter, "confidence": "medium",
...
def run_arjun_scan(
    ...
    with tempfile.TemporaryDirectory(prefix="arjun-priority-") as td:
        ...
        parameters=sorted(
            set(found) - set(known), key=lambda value:(value.lower() not in HIGH_RISK_NAMES,value.lower()),
        )
        findings=[_finding(target_url,p,method) for p in parameters]
        common={
            "vulnerabilities":findings, "parameters":parameters,
            "known_parameters":known, "authenticated":bool(cookies), "hard_failure":phase.get("status")=="error",
            "phases":[{
                "name":phase.get("phase"), "status":phase.get("status"),
                "diagnosis":phase.get("diagnosis",""), "return_code":phase.get("return_code"),
                "parameters":phase.get("phase_parameters",0), "timeout_seconds":phase.get("phase_timeout_seconds"),
                "command":phase.get("command",[]), "stdout_excerpt":str(phase.get("stdout",""))[-3000:],
                "stderr_excerpt":str(phase.get("stderr",""))[-3000:], "output_excerpt":str(phase.get("output",""))[-1500:],
            }],
            "scan_profile":f"bounded high-risk hidden-parameter names; method={method}", "request_method":method,
        ...
        if phase.get("status")=="error" and not parameters:
            ...
            result=failure(
                "Arjun", target_url,
                f"Arjun failed before returning parameters. {detail[-1200:]}", return_code=phase.get("return_code"),
                stdout=str(phase.get("stdout") or ""), stderr=str(phase.get("stderr") or ""),
                diagnosis=str(phase.get("diagnosis") or "scanner_error"), command=phase.get("command",[]),
            )
            result.update(common)
            return result
        if phase.get("status")=="partial":
            return partial(
                "Arjun",target_url, f"Arjun ended with incomplete coverage. Hidden parameters preserved: {len(parameters)}.",
                diagnosis="time_limit_reached",timed_out=True,**common,
            )
        return success(
            "Arjun",target_url, f"Arjun completed for {method}. New hidden parameters: {len(parameters)}.", **common,
        )
